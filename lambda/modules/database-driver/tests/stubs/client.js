// DynamoDB Document Client Stub
module.export = {
  query: (params, callback) => {
    const tableName = params.TableName;
    const indexName = params.IndexName;
    const keyConditionExpression = params.KeyConditionExpression;
    const expressionAttributeValues = params.ExpressionAttributeValues;

    if (!dbEntries.hasOwnProperty(tableName)) {
      callback(0);
    }
    let keysExpressions = [];
    let conditions = keyConditionExpression.split(" ");
    let key = 

    else if (keyConditionExpression === "unique_name = :hkey and version = :rkey")
    {
      TableName: tableName,
...
class KeyExpression {
  ...
  equals(entity) {
    if (entity.hasOwnProperty(this.key)) {
      if (entity[key.value] === this.value) {
        return true
      }
    }
    return false;
  }
  // Will add more if query service expands beyond current functionality
}
