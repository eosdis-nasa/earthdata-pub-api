...
describe('submission', () => {
    ...
    it('should submit a form', async () => {
        ...
        const payload_id = {id: 'test id', ...payload_no_id};
        db.user.findById.mockReturnValue({id: 'test user'});
        ...
        expect(await submission.handler(payload_id)).toEqual({
            id: 'test submission',
            conversation_id: 'test conversation',
            workflow_id: 'test workflow',
            step: {name: 'test step'},
        ...
        expect(await submission.handler(payload_no_id)).toEqual({
            id: 'test submission',
            conversation_id: 'test conversation',
            workflow_id: 'test workflow',
            step: {step_message: 'test message'},
    ...
    it('should be able to copy a submission', async () => {
        ...
        db.submission.copyFormData.mockImplementationOnce(({id, data, origin_id}) => {
            expect(data).toEqual(JSON.stringify(testFormData));
            expect(id).toEqual('copy test submission');
            expect(origin_id).toEqual('origin test id');
        });
        ...
        db.submission.setSubmissionCopy.mockImplementation(({id, edpuser_id, origin_id,context})=>{
            expect(id).toEqual('copy test submission');
            expect(edpuser_id).toEqual('test user');
            expect(origin_id).toEqual('origin test id');
