...
describe('notification-consumer', () => {
    ...
    it('should process a step notification', async () => {
        ...
        msg.parseRecord.mockImplementation((params) => {
            return {eventMessage: JSON.parse(params.Sns.Message)}
        })
        db.user.findSystemUser.mockImplementation(async(params) => {return {id: '1043c36e-3b6b-48c3-b1fa-8277fe65589f'}})
        process.env.AWS_EXECUTION_ENV = 'test'
        db.workflow.getLongName.mockImplementation(async(params) => {return {long_name: 'test workflow'}})
        db.submission.getFormData.mockImplementation(async(params) => {return {data: {data_product_name_value: 'test product'}}})
        db.submission.getStepName.mockImplementation(async(params) => {return {step_name: 'test step'}})
        db.submission.getCreatorName.mockImplementation(async(params) => {return {name: 'test user'}})
        db.note.reply.mockImplementation(async(params) => {return {conversation_id : '1043c36e-3b6b-48c3-b1fa-8277fe65589f', sender_edpuser_id: '1043c36e-3b6b-48c3-b1fa-8277fe65589f'}})
        db.note.getEmails.mockImplementation(async(params) => {return [{email: 'test@test.test'}]})
