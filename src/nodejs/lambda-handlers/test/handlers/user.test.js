...
describe('user', () => {
  ...
  it('should find user by id', async () => {
    db.user.findById.mockImplementationOnce(
      () => ({ user_privileges: ['USER_READ'] })
    ).mockImplementation((params) => params.id);
  ...
  it('should find user by name', async () => {
    ...
    db.user.find.mockImplementation((params) => params.name);
    const testEvent = {
      operation: 'find',
      context: { user_id: '958fab13-ae06-470b-80e0-c9ba4e60f1bc' },
      name: 'test'
  ...
  it('should add a user to the group', async () => {
    db.user.findById.mockImplementationOnce(
      () => ({ user_privileges: ['USER_ADDGROUP'] })
    ).mockImplementation(() => ({ user_groups: [{ id: '958fab13-ae06-470b-80e0-c9ba4e60f1bc' }] }));

    db.user.addGroup.mockImplementation((params) => params.group_id);
    const testEvent = {
      operation: 'add_group',
      context: { user_id: '958fab13-ae06-470b-80e0-c9ba4e60f1bc' },
  ...
  it('should remove a user from the group', async () => {
    db.user.findById.mockImplementationOnce(
      () => ({ user_privileges: ['USER_REMOVEGROUP'] })
    ).mockImplementation(() => ({ user_groups: [{ id: '958fab13-ae06-470b-80e0-c9ba4e60f1bc' }] }));

    db.user.removeGroup.mockImplementation((params) => params.group_id);
    const testEvent = {
  ...
  it('should add a role to the user', async () => {
    db.user.findById.mockImplementationOnce(
      () => ({ user_privileges: ['USER_ADDROLE'] })
    );
    db.role.findByName.mockImplementation(() => ({ id: '1fe623d5-c5ed-47b2-98ce-c40577cf6872' }));
    db.user.addRole.mockImplementation((params) => params.role_id);
    const testEvent = {
      operation: 'add_role',
      context: { user_id: '958fab13-ae06-470b-80e0-c9ba4e60f1bc' },
  ...
  it('should remove a role from the user', async () => {
    db.user.findById.mockImplementationOnce(
      () => ({ user_privileges: ['USER_REMOVEROLE'] })
    );
    db.user.removeRole.mockImplementation((params) => params.role_id);
    const testEvent = {
  ...
  it('should get user names bassed on a list of ids', async () => {
    db.user.getUsers.mockImplementation((params) => params.ids);
    const testEvent = {
      operation: 'get_users',
      context: { user_id: '958fab13-ae06-470b-80e0-c9ba4e60f1bc' },
