...
async function handler(event) {
  const { code, refresh, logout, state, context } = event;
  if (code) {
  ...
  else if (refresh && context) {
    const { refresh_token: refreshToken } = await DatabaseUtil.execute({
      resource: 'user', operation: 'getRefreshToken' },
